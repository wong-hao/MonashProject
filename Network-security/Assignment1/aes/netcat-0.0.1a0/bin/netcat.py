...
g = 2  # parameter of Diffie-Hellman algorithm
p = 65537  # parameter of Diffie-Hellman algorithm
...
def send(s, udp=False, crlf=False, verbose=0):
    ...
    while True:
        ...
        if i == 1:  # Diffie-Hellman algorithm
            global privatekey
            global g
            global p
            print("Sent p: ", p)
            publickey = g ** privatekey % p
            data = str(publickey).encode() + b'\n'
            print("Sent public key: ", data)
            key = (publickey ** privatekey % p).to_bytes(16, 'big')
            print("Actual session key: ", key)
            print("\n")
            print("The  key agreement scheme to decide a session key succeed! Please communicate now!\n")


        else:  # AES algorithm
            cipher = AES.new(key, AES.MODE_EAX)
            nonce = cipher.nonce  # include use of a salt value which is added with the ciphertext (ciphertext + nonceSalt)

            data = data.encode()  # transfer string to bytes
            # include use of a salt value which is added with the ciphertext (ciphertext + tagSalt)
            ciphertext, tag = cipher.encrypt_and_digest(data)
            # generate triple (nonce, ciphertext, tag)
            ...
            data = triple_base64  # send the base64ed triple
...
def receive(s, udp=False, bufsize=1024, verbose=0):
    ...
    while True:
        ...
        if j == 1:  # Diffie-Hellman algorithm
            print("The received public key: ", data)
            global privatekey
            global p
            print("Received p: ", p)
            publickey = int(data)
            key = (publickey ** privatekey % p).to_bytes(16, 'big')
            print("The Actual session key: ", key)
            print("\n")

        else:  # AES algorithm

            print("The receive data: ", data)
            triple1 = base64.b64decode(data)  # receive the base64ed triple and debase64 it

            nonce_loc = triple1.find(b'nonce')  # find the start location of nonce in the debase64ed triple
            # find the start location of ciphertext in the debase64ed triple
            ciphertext_loc = triple1.find(b'ciphertext')
            tag_loc = triple1.find(b'tag')  # find the start location of tag in the debase64ed triple

            nonce1 = triple1[nonce_loc + len("nonce") + 2:ciphertext_loc]  # cut the nonce from the debase64ed triple
            ciphertext1 = triple1[ciphertext_loc + len(
                "ciphertext") + 2:tag_loc]  # cut the ciphertext from the debase64ed triple
            tag1 = triple1[tag_loc + len("tag"):len(triple1)]  # cut the tag from the debase64ed triple
