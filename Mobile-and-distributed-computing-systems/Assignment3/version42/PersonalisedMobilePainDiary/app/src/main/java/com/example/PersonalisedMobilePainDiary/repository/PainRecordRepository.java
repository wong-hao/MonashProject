...
public class PainRecordRepository {
    ...
    public void updatePainrecordByID(final int painrecordID, final int intensity,
        ...
        PainRecordDatabase.databaseWriteExecutor.execute(new Runnable() {
            @Override
            public void run() {
                dao.updateByID(painrecordID, intensity, location, mood, steps,date, temperature, humidity, pressure, email);
            }
        });
    }

    public void updatePainrecordByDate(final String date2, final int intensity,
                                     final String location, final String mood, final int steps, final String date, final String temperature, final String humidity, final String pressure, final String email) {
        PainRecordDatabase.databaseWriteExecutor.execute(new Runnable() {
            @Override
            public void run() {
                dao.updateByDate(date2, intensity, location, mood, steps,date, temperature, humidity, pressure, email);
            }
        });
    }
